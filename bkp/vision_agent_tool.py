...
def make_json_serializable(obj):
    ...
    if isinstance(obj, (np.integer, np.intc, np.intp, np.int8,
                        ...
                        np.uint32, np.uint64)):
        return int(obj)
    # Check for numpy float types (NumPy 2.0 compatible - np.float_ removed)
...
def analyze_claim_image(image_path: str) -> dict:
    ...
    # )
    damage_consistency = "high" if (
        vehicle_detected and
        "damage" in caption.lower()
    ) else "unknown"
    ...
    extracted_data = {
        ...
        "authenticity_signals": {
            ...
            "ai_confidence_not_determinable": 'true'
        }
    }
